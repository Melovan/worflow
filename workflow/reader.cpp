...
ConfigReader::ConfigReader(const string& filename) {
    ...
    if (!file.is_open()) throw runtime_error("ConfigReader : Input file can not be opened");
    if (file.peek() == std::ifstream::traits_type::eof()) throw runtime_error("ConfigReader : Void into input file");
}
